...
function createRuntime(deps, options = {}) {
  ...
  function connectWs(config) {
    ...
    const printToken = getTenantPrintToken(config.empresaId);
    const wsUrl = printToken
      ? `${cfg.wsEndpoint}?tenant=${encodeURIComponent(config.empresaId)}&token=${encodeURIComponent(printToken)}`
      : `${cfg.wsEndpoint}?tenant=${encodeURIComponent(config.empresaId)}`;
  ...
  function getTenantPrintToken(tenant) {
    if (!cfg.printDriverToken || !tenant) return '';
    return crypto
      .createHmac('sha256', cfg.printDriverToken)
