...
function createRuntime(deps, options = {}) {
  ...
  // Una opción explícita vacía deshabilita el token raíz para escenarios
  // controlados y pruebas de compatibilidad; no debe ser reemplazada por un
  // secreto heredado del entorno mediante `||`.
  const hasExplicitPrintDriverToken = Object.prototype.hasOwnProperty.call(options, 'printDriverToken');
  const configuredPrintDriverToken = hasExplicitPrintDriverToken
    ? options.printDriverToken
    : (process.env.IDA_PRINT_DRIVER_TOKEN || process.env.SECURITY_PRINT_DRIVER_TOKEN || '');
  ...
  const cfg = {
    ...
    maxPrintRetries: options.maxPrintRetries || 5,
    printDriverToken: configuredPrintDriverToken || '',
    pendingAckPersistDebounceMs: options.pendingAckPersistDebounceMs || 250,
    requireJobId: options.requireJobId !== false,
    enforcePrinterAllowlist: options.enforcePrinterAllowlist === true
  };

  const resolvePrintDriverToken = () => {
    if (typeof getPrintDriverToken === 'function') return getPrintDriverToken() || '';
    return cfg.printDriverToken;
  };

  const resolveDriverAccessId = () => typeof getDriverAccessId === 'function' ? getDriverAccessId() || '' : '';
  ...
  let allowedPrinterKeys = new Set();
  ...
  function isPrinterAllowed(ip, puerto) {
    if (!cfg.enforcePrinterAllowlist) return true;
    if (!allowedPrinterKeys || allowedPrinterKeys.size === 0) return true;
    return allowedPrinterKeys.has(queueKey(ip, puerto));
  }
  ...
  function quarantineJob(job, reason, source) {
    const entry = {
      ...
      key: job && job.ip ? queueKey(job.ip, (job && job.puerto) || 9100) : null,
  ...
  function normalizeIncomingJob(job, source) {
    ...
    if (!isPrinterAllowed(normalized.ip, normalized.puerto)) {
      quarantineJob(job, 'printer-not-allowed', source);
      return null;
    }
  ...
  function addToQueue(ip, puerto, texto, jobId = null) {
    ...
    if (!isPrinterAllowed(ip, puerto || 9100)) {
      quarantineJob({ ip, puerto, texto, id: jobId }, 'printer-not-allowed', 'direct');
      return { accepted: false, reason: 'printer-not-allowed' };
    }
  ...
  async function processQueue(key) {
    ...
    try {
      ...
      if (printer.retryCount >= cfg.maxPrintRetries) {
        quarantineJob(item, 'max-print-retries', 'print');
        if (item.jobId && typeof markAsFailed === 'function') {
          try {
            await markAsFailed(item.jobId, status.printers[key].lastError);
            audit('error_ack_ok', { jobId: item.jobId });
          } catch (ackError) {
            audit('error_ack_error', {
              jobId: item.jobId,
              error: ackError && ackError.message ? ackError.message : String(ackError)
            });
          }
        }
      } else {
  ...
  async function syncPendingJobs() {
    if ((!status.tenant && !resolveDriverAccessId()) || typeof fetchPendingJobs !== 'function') return;
    ...
    try {
      ...
      status.lastSyncError = err && err.message ? err.message : 'sync-failed';
      status.lastError = status.lastSyncError;
      if (logger && typeof logger.error === 'function') {
  ...
  function connectWs(config) {
    const endpoint = typeof getWsEndpoint === 'function' ? getWsEndpoint() : cfg.wsEndpoint;
    if (stopped || !endpoint || typeof createWebSocket !== 'function') return;
    const accessId = resolveDriverAccessId(config);
    const printToken = getTenantPrintToken(config.empresaId, resolvePrintDriverToken());
    const wsUrl = accessId
      ? `${endpoint}?driverAccessId=${encodeURIComponent(accessId)}`
      : printToken
      ? `${endpoint}?tenant=${encodeURIComponent(config.empresaId)}&token=${encodeURIComponent(printToken)}`
      : `${endpoint}?tenant=${encodeURIComponent(config.empresaId)}`;
    ...
    ws.on('error', (err) => {
      ...
      status.lastError = err && err.message ? err.message : 'websocket-error';
      emitState();
      if (logger && typeof logger.error === 'function') {
  ...
  function getTenantPrintToken(tenant, rootSecret = resolvePrintDriverToken()) {
  ...
  function start(config) {
    ...
    allowedPrinterKeys = new Set(
      Array.isArray(config && config.impresoras)
        ? config.impresoras
          .filter((p) => p && p.ip)
          .map((p) => queueKey(p.ip, p.puerto || 9100))
        : []
    );
    emitState();
    connectWs(config);
